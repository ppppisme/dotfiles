...
sub checkactive {
	...
	my $key = $channel.'@'.$nick;
	if (!exists $lastmsg->{$key} || $lastmsg->{$key} <= time() - Irssi::settings_get_int('smartfilter_delay')) {
		delete $lastmsg->{$key};
		Irssi::signal_stop();
	}

	if (exists $lastmsg->{$key} && $altnick) {
		$lastmsg->{$channel.'@'.$altnick} = $lastmsg->{$key};
		delete $lastmsg->{$key};
	}
